...
class Admin extends CI_Controller {
	...
	public function searchDataSnumber(){
		if($_SESSION['administrator'] != ''){
			$data = array(
				'Snumber' => $this->input->post('Snumber'),
			);
			...
			$this->load->model('administrator_model', 'administrator');
			//获取查询维修信息
			$list = $this->administrator->getUserSerachSnumber($data);
			//跳转界面
			$this->load->view('admin/searchData', array('list' => $list));
		}else{
			$url = site_url('admin/index');
	...
	public function searchDataUser(){
		if($_SESSION['administrator'] != ''){
			...
			$this->load->model('administrator_model', 'administrator');
			//获取查询维修信息
			$list = $this->administrator->getUserSerachUser($data);
			//跳转界面
			$this->load->view('admin/userMessage', array('list' => $list));
		}else{
			$url = site_url('admin/index');
			echo "<script language='javascript' type='text/javascript'>";
			echo "window.location.href='$url'";
